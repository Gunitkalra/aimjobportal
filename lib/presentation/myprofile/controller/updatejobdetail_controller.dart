...
import '../model/updatejobdetail_Model.dart';
import 'getProfile_Controller.dart';

class UpdateJobDetailController extends GetxController {
  ...
  Future<void> updateJobDetails({
    ...
    try {
      ...
      final url = Uri.parse("${ApiList.baseUrl}/v1/profile/job-details");
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final res = UpdateJobDetailResponseModel.fromJson(json.decode(response.body));
        if (res.success == true) {
          showToastSuccess("Job details updated successfully.");
          
          if (Get.isRegistered<GetProfileController>()) {
            Get.find<GetProfileController>().fetchProfile();
          }
        } else {
          showToastFail(res.message ?? "Failed to update job details.");
        }
      } else {
        showToastFail("Error: ${response.statusCode}");
      }
    } catch (e) {
      print("Update Job Details Error: $e");
