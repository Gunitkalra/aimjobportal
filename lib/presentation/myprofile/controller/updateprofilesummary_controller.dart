...
import '../model/updateprofilesummary_Model.dart';
import 'getProfile_Controller.dart';

class UpdateProfileSummaryController extends GetxController {
  ...
  Future<void> updateProfileSummary({
    ...
    try {
      ...
      final url = Uri.parse("${ApiList.baseUrl}/v1/profile/summary");
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final res = UpdateProfileSummaryResponseModel.fromJson(json.decode(response.body));
        if (res.success == true) {
          showToastSuccess("Profile summary updated successfully.");
          
          if (Get.isRegistered<GetProfileController>()) {
            Get.find<GetProfileController>().fetchProfile();
          }
        } else {
          showToastFail(res.message ?? "Failed to update profile summary.");
        }
      } else {
        showToastFail("Error: ${response.statusCode}");
      }
    } catch (e) {
      print("Update Profile Summary Error: $e");
