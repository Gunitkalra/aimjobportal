...
class SideDashboardScreen extends GetView<SideDashboardController> {
  const SideDashboardScreen({super.key});

  @override
  Widget build(BuildContext context) {
    // Initialize controller explicitly
    final controller = Get.put(SideDashboardController());
    final sw = MediaQuery.of(context).size.width;
...
class _WelcomeCard extends StatelessWidget {
  final SideDashboardController controller;
  const _WelcomeCard({required this.controller});
...
class _ProfileCompletionCard extends StatelessWidget {
  final dynamic data;
  const _ProfileCompletionCard({required this.data});
...
// ── Complete your profile card (Missing Fields) ──────────────────────────────

class _CompleteProfileCard extends StatelessWidget {
  final dynamic data;
  const _CompleteProfileCard({required this.data});
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(a.activityType ?? "", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                        const SizedBox(height: 2),
                        Text(a.description ?? "", style: const TextStyle(fontSize: 12, color: AppColors.textSecondary, height: 1.4)),
                        const SizedBox(height: 4),
                        Text(_formatTime(a.timestamp), style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
                      ],
                    ),
                  ),
                ],
              ),
              if (i < activities.length - 1) const Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Divider(height: 1, color: Color(0xFFEEEEEE))),
            ],
          );
        }),
      ),
